...
export const MUTATIONS = [
  ...
  },
  {
    ...
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'a clamped ghost gives a zero end tangent, so the surface flattens into every border of the net — the identity map stops being the identity by ~3e-2 of the frame in the boundary cells, which reads as the image going slack at the edges rather than as a maths error. Re-anchored on the ghost helper: the original pattern named _ctl\'s inlined arithmetic, which the quadratic end condition factored out, and the harness reported it as matching nothing rather than letting it quietly cover nothing',
    find: 'function _ghostQ(a, b, c) {\n  return { x: 3 * a.x - 3 * b.x + c.x, y: 3 * a.y - 3 * b.y + c.y };',
    replace: 'function _ghostQ(a, b, c) {\n  return { x: a.x, y: a.y };',
  },
  {
  ...
  },
  {
    ...
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'an approximating basis instead of an interpolating one — the control points stop being where the image lands, which is the entire job of a calibration mesh, and it looks like a mesh that is merely a bit soft rather than one that is wrong. Anchored on the LIVE basis: the first version broke a leftover Catmull-Rom helper that the Hermite rewrite had orphaned, so it mutated unreachable code and survived',
    find: '  return [2 * t3 - 3 * t2 + 1, -2 * t3 + 3 * t2];',
    replace: '  return [2 * t3 - 3 * t2 + 0.98, -2 * t3 + 3 * t2];',
  },
  {
    name: 'projmesh: the twist term is dropped from the patch',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'zeroing the cross-derivative is the usual shortcut when moving to a Hermite basis (a Ferguson patch) and it does NOT reproduce Catmull-Rom — every mesh saved before handles existed would open subtly reshaped, which on a physical surface means re-aligning a mapping that was already correct',
    find: '    const a = this._ctl(i + 1, j + 1), b = this._ctl(i - 1, j + 1);',
    replace: '    if (true) return { x: 0, y: 0 };\n    const a = this._ctl(i + 1, j + 1), b = this._ctl(i - 1, j + 1);',
  },
  {
    name: 'projmesh: a dragged handle is ignored by the sampler',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'the handle moves, the arm follows it, the badge fills in — and the surface does not bend, which reads as the feature being subtle rather than dead',
    find: '    const t = this.tans[this.idx(i, j)]?.[axis];\n    if (t) return { x: t[0], y: t[1], explicit: true };',
    replace: '    const t = null;\n    if (t) return { x: t[0], y: t[1], explicit: true };',
  },
  {
    name: 'projmesh: clearing a handle leaves the override behind',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'double-click is the ONLY way back to the derived curve, so a clear that does not clear means a handle can never be undone and the shape has to be found again by eye',
    find: '    delete e[axis];',
    replace: '',
  },
  {
    name: 'projmesh: handles are not saved',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'a shape takes real time to dial in against a physical object; losing the handles on save means the slot recalls a mesh that is close but wrong, which is worse than one that is obviously empty',
    find: '    if (this.tangentCount) {',
    replace: '    if (false) {',
  },
  {
    name: 'projmesh: a resize re-keys handles onto whatever point inherits the index',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'index 4 of a 3x3 is the centre and index 4 of a 5x5 is on the top row, so carrying overrides through setGrid bends the surface at points the user never touched — and it happens on a control (Mesh Cols) documented as never moving a pixel',
    find: '    this.tans = {};\n    this._changed();\n    return true;\n  }\n\n  /** Move one control point',
    replace: '    this._changed();\n    return true;\n  }\n\n  /** Move one control point',
  },
  {
    name: 'projmesh: a crossfade keeps the handles it started with',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'fading to a shape that carries no handles would leave it holding a frozen copy of the previous shape\'s curvature, so the slot recalls as something neither mesh ever looked like — and only on the second recall, which makes it read as random',
    find: '      if (m.tanLand) this.tans = m.tanLand;',
    replace: '',
  },
  {
    name: 'projmesh: a loaded file can park a handle outside the net',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'a mesh written by a build with a different net, or edited by hand, would carry tangents indexed past the last point; they are invisible, they survive every save, and tangentCount reports them as real',
    find: '        if (!Number.isInteger(i) || i < 0 || i >= n || !e) continue;',
    replace: '        if (!e) continue;',
  },
  {
    name: 'projmesh: the 2-point fallback clamps instead of extrapolating',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'the linear ghost is the only end condition a 2-wide or 2-tall net has, and clamping halves its end tangent so the image compresses toward that border — invisible in every other fixture here, because they all have three or more columns',
    find: 'function _ghostL(a, b) {\n  return { x: 2 * a.x - b.x, y: 2 * a.y - b.y };',
    replace: 'function _ghostL(a, b) {\n  return { x: a.x, y: a.y };',
  },
  {
    name: 'projmesh: the end tangent is the chord again',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'THE SHIPPED DEFECT. With a chord end condition an edge leaves each corner aimed straight at the next point and does all its bending in the middle — turn-rate variation 12.2x against a circle\'s 1.0x. The owner reported it as "not making a perfect bow when wanted, has a bit of a corner to it", which is what 12x looks like. Nothing errors and every handle still works',
    find: '      return C >= 3 ? _ghostQ(this._ctl(0, j), this._ctl(1, j), this._ctl(2, j))\n                    : _ghostL(this._ctl(0, j), this._ctl(1, j));',
    replace: '      return _ghostL(this._ctl(0, j), this._ctl(1, j));',
  },
  {
    name: 'projmesh: only the mesh path sets the edge-fade uniforms',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/main.js',
    why: 'render() and renderMesh() share one program, so a uniform left unset in one path keeps whatever the OTHER path last wrote — the edge blend would work on a mesh and be stale or absent on a bare 4-corner pin, which is the configuration a first calibration starts in',
    find: '      gl.uniform1f(locFade,fade); gl.uniform1f(locGamma,gamma);\n      gl.drawArrays(gl.TRIANGLES,0,6);',
    replace: '      gl.drawArrays(gl.TRIANGLES,0,6);',
  },
  {
    name: 'projmesh: the edge fade is a screen-space band',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/main.js',
    why: 'computing the falloff from anything but uv makes it a rectangle in screen space, so on a warped or curved mesh the soft edge cuts across the picture instead of following its border — and it looks like a bug in the mapping rather than in the fade',
    find: "'vec2 d=min(uv,1.0-uv)/uFade;'+",
    replace: "'vec2 d=min(gl_FragCoord.xy/vec2(1920.0,1080.0),1.0)/uFade;'+",
  },
  {
    name: 'projmesh: the posted fade is trusted instead of clamped',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/main.js',
    why: 'uFade is a DIVISOR in the shader, so a 0 arriving from a stale or hand-edited message divides by zero and a negative inverts the ramp — the band would fill the whole image with black instead of feathering its border',
    find: 'GL.setEdge(Math.max(0,Math.min(0.5,(+e.data.edgeFade||0)/100)),',
    replace: 'GL.setEdge((+e.data.edgeFade||0)/100,',
  },
  {
    name: 'projmesh: the fold guard is back to testing arithmetic, not geometry',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'THE SHIPPED DEFECT. |w| < 1e-12 tests for a division that has already failed; by |w| = 1e-3 the surface is seventeen screen-widths across. The owner hit this as a fan of lines and a smeared image, and the only recovery was to notice that raising Mesh Curve avoided it',
    find: '  const m = foldMargin(g, h);\n  if (m <= W_FLOOR) return quadAt(tl, tr, br, bl, u, v);',
    replace: '  const m = 1;',
  },
  {
    name: 'projmesh: a near-folding cell is left untessellated',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'the output window draws the raw control cells at sub 1 with its OWN perspective correction, so the fold guard in homographyAt is never asked to sample and cannot fire — the guard would be present, correct, and completely bypassed on exactly the meshes it exists for',
    find: '    const risky = this.worstMargin() < W_KNEE;\n    if (!risky && (!(this.curve > 0) || this.isQuad)) return 1;',
    replace: '    if (!(this.curve > 0) || this.isQuad) return 1;',
  },
  {
    name: 'projmesh: the fold guard switches instead of blending',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'a hard switch pops the whole picture the frame a cell crosses the knee, and the knee is a threshold a performer drags through — a jump mid-set reads as the mapping breaking, not as a guard working',
    find: '  const s = t * t * (3 - 2 * t);              // smoothstep: C1 at both ends',
    replace: '  const s = 1;',
  },
  {
    name: 'projmesh: arrow keys always move the point, never the handle',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/main.js',
    why: 'the handle is the more delicate control — it sits at P + T/3, so a pixel of drag is three pixels of tangent — and it would be the one thing on the mesh with no fine adjustment at all',
    find: '    if(selTan)nudgeTangent(d[0],d[1]); else nudgePoint(d[0],d[1]);',
    replace: '    nudgePoint(d[0],d[1]);',
  },
  {
    name: 'projmesh: the handle selection outlives the point selection',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/main.js',
    why: 'select another point and the arrow keys would go on editing the curve of the one you left — invisible, because the handle you are changing is no longer the one on screen',
    find: '    selTan=null;\n    positionHandles();',
    replace: '    positionHandles();',
  },
  {
    name: 'projmesh: a non-finite tangent is stored',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'THE SHIPPED BUG. An Infinity from dividing by a zero viewport put 456 of 625 render-net vertices non-finite, and a Float32Array of NaN in drawArrays is a driver fault — Chrome runs one GPU process per browser, so the output window took the MAIN window context down with it and the instrument went black from one handle drag',
    find: '    if (!Number.isFinite(dx) || !Number.isFinite(dy)) return false;',
    replace: '',
  },
  {
    name: 'projmesh: the vertex boundary lets a non-finite sample through',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'the last line before a GPU buffer; every producer upstream is guarded and this exists because a non-finite vertex is not a wrong picture, it is a fault that takes every WebGL context in the browser with it',
    find: '        if (Number.isFinite(p.x) && Number.isFinite(p.y)) {',
    replace: '        if (true) {',
  },
  {
    name: 'projmesh: a loaded file can carry a non-finite tangent',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/inputs/ProjMapMesh.js',
    why: 'JSON has no NaN so a bad tangent round-trips as null, but a hand edit, a future format or an in-memory copy that never went through JSON all can carry one — and a single non-finite tangent is enough to fault the GPU on the next frame',
    find: '                           Number.isFinite(+a[0]) && Number.isFinite(+a[1]);',
    replace: '                           true;',
  },
  {
    name: 'projmesh: the output window draws whatever it is handed',
    audit: 'audit-projmap-curve.mjs',
    file: 'src/main.js',
    why: 'the defence closest to the driver; falling back to the CSS path is a worse picture, and drawing a NaN vertex is a dead browser',
    find: '        if(!isFinite(pos[n])){ note(\'non-finite-vertex\'); return false; }',
    replace: '',
  },
  {
